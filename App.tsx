...
const TEMPLATE_URLS = {
  cert1: 'https://raw.githubusercontent.com/harishmerwade/KRV/bbf8a68087298751e1d9f9507d4abd5af7f7213a/Img/temp%201.png',
  cert2: 'https://raw.githubusercontent.com/harishmerwade/KRV/bbf8a68087298751e1d9f9507d4abd5af7f7213a/Img/temp%202.png',
  id_front: 'https://raw.githubusercontent.com/harishmerwade/KRV/9f45cb47c6e17235b64d4381374692296202852b/Img/1.png',
  id_back: 'https://raw.githubusercontent.com/harishmerwade/KRV/82111b99eb25d019b7c196bcfc76b898c133579b/Img/2.png'
};
...
const INITIAL_LAYOUT_CERT1: FullLayout = {
  sides: [{
    ...
    textItems: [
      { id: "pledge_text_1", text: "ಹೆಚ್. ಶಿವರಾಮೇಗೌಡರ ಕರ್ನಾಟಕ ರಕ್ಷಣಾ ವೇದಿಕೆಯ ರಾಜ್ಯಾಧ್ಯಕ್ಷರ ನೇತೃತ್ವದಲ್ಲಿ ಸತ್ಯ, ನ್ಯಾಯ-ಧರ್ಮವನ್ನು ಪಾಲಿಸಿಕೊಂಡು, ಆತ್ಮಪೂರ್ವಕವಾಗಿ ಈ ಕೆಳಕಂಡ ಪ್ರತಿಜ್ಞಾವಿಧಿಯ ಪ್ರಮಾಣವಚನ ಸ್ವೀಕರಿಸುತ್ತಿದ್ದೇನೆ.", x: 259, y: 532, fontSize: 20, color: "#000000", isBold: true, isLocked: false, width: 674, lineHeight: 1.6, isVisible: true },
      { id: "pledge_text_2", text: "ಕನ್ನಡ ನಾಡು, ನುಡಿ, ನೆಲ, ಜಲ, ಗಡಿ, ಭาಷೆ ಸಂಸ್ಕೃತಿಯನ್ನು ಉಳಿಸಿ ಬೆಳೆಸುವ ಮಹತ್ತರ ಕಾರ್ಯದಲ್ಲಿ ನಾವೆಲ್ಲರೂ ಕನ್ನಡಾಂಬೆಯ ಸುಪುತ್ರ / ಸುಪುತ್ರಿಯರಾಗಿ ಒಮ್ಮತದಿಂದ ಶ್ರಮಿಸುತ್ತೇವೆ ಎಂದು ಈ ಮೂಲಕ ಪ್ರತಿಜ್ಞೆ ಮಾಡಿ ಕೆಳಗೆ ಸಹಿ ಹಾಕಿರುತ್ತೇನೆ.", x: 41.25, y: 632, fontSize: 20, color: "#000000", isBold: true, isLocked: false, width: 912, lineHeight: 1.6, isVisible: true },
...
const App: React.FC<AppProps> = ({ onLogout }) => {
  ...
  const [bottomPhotoData, setBottomPhotoData] = useState<string | undefined>(undefined);
  ...
  const updateItem = (sideIdx: number, id: string, updates: any) => {
    setLayouts(prev => {
      ...
      } else if (id === 'bottom_photo') {
        side.photo2 = { ...side.photo2, ...updates } as PhotoItem;
      } else {
  ...
  const handleDrag = (sideIdx: number, id: string, e: any) => {
    ...
    if (!side) return;
    const item = id === 'member_photo' 
      ? side.photo 
      : (id === 'bottom_photo' ? side.photo2 : side.textItems.find(i => i.id === id));
  ...
  const selectedItemData = selectedElement ? (
    ...
    : (selectedElement.id === 'bottom_photo'
       ? currentLayout.sides[selectedElement.sideIdx]?.photo2
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-thin">
          ...
          <section className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {activeTemplate.startsWith('cert') && (
                <>
                  <div className="space-y-1">
                    <label className="text-[8px] font-black text-slate-500 uppercase ml-1">President / Signatory Name</label>
                    <input type="text" value={certificateData.presidentName || ''} onChange={e => setCertificateData(p => ({...p, presidentName: e.target.value}))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-red-600 transition-all" />
                  </div>
                  <div className="space-y-1">
            ...
            {activeTemplate.startsWith('cert') && (
              <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200 cursor-pointer" onClick={() => setCertificateData(p => ({...p, showBottomPhoto: !p.showBottomPhoto}))}>
                <div className="flex items-center gap-3">{certificateData.showBottomPhoto ? <Eye size={18} className="text-emerald-500"/> : <EyeOff size={18} className="text-slate-400"/>}<span className="text-[10px] font-black uppercase text-slate-600">Show Bottom Photo instead of Date Info</span></div>
                <div className={`w-10 h-5 rounded-full relative transition-colors ${certificateData.showBottomPhoto ? 'bg-emerald-500' : 'bg-slate-300'}`}><div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${certificateData.showBottomPhoto ? 'left-6' : 'left-1'}`} /></div>
              </div>
            )}
          </section>
          ...
          {activeTemplate.startsWith('cert') && certificateData.showBottomPhoto && (
            <section className="space-y-4 pt-4 border-t border-slate-100">
              <div className="flex items-center gap-2 text-slate-400"><ImageIcon size={14} /><h2 className="text-[10px] font-black uppercase tracking-widest">Bottom Photo (No Frame)</h2></div>
              <label className="cursor-pointer p-6 rounded-2xl border-2 border-dashed border-slate-200 text-center transition-all block group">
                <ImageIcon className="mx-auto mb-2 text-slate-300 group-hover:text-red-600 transition-colors" size={24} /><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Upload Bottom Photo</p>
                <input type="file" className="hidden" accept="image/*" onChange={e => {
                  const file = e.target.files?.[0];
                  if (file) {
                    const reader = new FileReader();
                    reader.onload = (event) => setBottomPhotoData(event.target?.result as string);
                    reader.readAsDataURL(file);
                  }
                }} />
              </label>
          ...
          {selectedElement && selectedItemData && (
            <section className="bg-slate-900 p-5 rounded-2xl space-y-4 animate-in">
              ...
              {['member_photo', 'bottom_photo'].includes(selectedElement.id) ? (
                <div className="space-y-4">
                  <div className="space-y-1"><div className="flex justify-between text-[8px] font-black text-slate-500 uppercase"><span>Zoom</span><span className="text-white">{(selectedItemData as PhotoItem).imageScale.toFixed(2)}x</span></div><input type="range" min="0.1" max="3" step="0.01" value={(selectedItemData as PhotoItem).imageScale} onChange={e => updateItem(selectedElement.sideIdx, selectedElement.id, {imageScale: +e.target.value})} className="w-full accent-red-600" /></div>
                  <div className="space-y-1"><div className="flex justify-between text-[8px] font-black text-slate-500 uppercase"><span>Horizontal Pos</span><span className="text-white">{(selectedItemData as PhotoItem).imageOffsetX}px</span></div><input type="range" min="-500" max="500" value={(selectedItemData as PhotoItem).imageOffsetX} onChange={e => updateItem(selectedElement.sideIdx, selectedElement.id, {imageOffsetX: +e.target.value})} className="w-full accent-red-600" /></div>
                  <div className="space-y-1"><div className="flex justify-between text-[8px] font-black text-slate-500 uppercase"><span>Vertical Pos</span><span className="text-white">{(selectedItemData as PhotoItem).imageOffsetY}px</span></div><input type="range" min="-500" max="500" value={(selectedItemData as PhotoItem).imageOffsetY} onChange={e => updateItem(selectedElement.sideIdx, selectedElement.id, {imageOffsetY: +e.target.value})} className="w-full accent-red-600" /></div>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <div className="space-y-1"><div className="flex justify-between text-[8px] font-black text-slate-500 uppercase"><span>Horizontal Pos (X)</span><span className="text-white">{(selectedItemData as TextItem).x.toFixed(0)}px</span></div><input type="range" min="-100" max={currentLayout.sides[selectedElement.sideIdx]?.canvasWidth || 1000} value={(selectedItemData as TextItem).x} onChange={e => updateItem(selectedElement.sideIdx, selectedElement.id, {x: +e.target.value})} className="w-full accent-red-600" /></div>
                  <div className="space-y-1"><div className="flex justify-between text-[8px] font-black text-slate-500 uppercase"><span>Vertical Pos (Y)</span><span className="text-white">{(selectedItemData as TextItem).y.toFixed(0)}px</span></div><input type="range" min="-100" max={currentLayout.sides[selectedElement.sideIdx]?.canvasHeight || 1414} value={(selectedItemData as TextItem).y} onChange={e => updateItem(selectedElement.sideIdx, selectedElement.id, {y: +e.target.value})} className="w-full accent-red-600" /></div>
        ...
        <div ref={workspaceRef} className="flex flex-col gap-10 transition-transform duration-500 items-center justify-center origin-center" style={{ transform: `scale(${canvasScale})` }}>
          {currentLayout?.sides?.map((side, sIdx) => (
            ...
            >
              ...
              {side.photo2 && certificateData.showBottomPhoto && (
                <div 
                  onMouseDown={e => { setSelectedElement({sideIdx: sIdx, id: 'bottom_photo'}); if(!isGlobalLocked && !side.photo2.isLocked) handleDrag(sIdx, 'bottom_photo', e); }}
                  className={`absolute ${selectedElement?.sideIdx === sIdx && selectedElement?.id === 'bottom_photo' ? 'ring-8 ring-red-600/50 z-40' : 'z-10'} ${!isGlobalLocked ? 'border-2 border-dashed border-red-500' : ''}`}
                  style={{ 
                    left: `${side.photo2.x}px`, 
                    top: `${side.photo2.y}px`, 
                    width: `${side.photo2.width}px`, 
                    height: `${side.photo2.height}px`, 
                    borderRadius: `${side.photo2.borderRadius}px`, 
                    cursor: isGlobalLocked ? 'default' : (side.photo2.isLocked ? 'not-allowed' : 'move'),
                    backgroundColor: bottomPhotoData ? 'transparent' : 'rgba(239, 68, 68, 0.1)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center'
                  }}
                >
                  {bottomPhotoData ? (
                    <img 
                      src={bottomPhotoData}
                      alt="bottom member photo"
                      style={{ 
                        maxWidth: '100%',
                        maxHeight: '100%',
                        objectFit: 'contain',
                        transform: `scale(${side.photo2.imageScale}) translate(${side.photo2.imageOffsetX}px, ${side.photo2.imageOffsetY}px)`,
                        pointerEvents: 'none'
                      }} 
                    />
                  ) : (
                    <span className="text-[10px] font-black text-red-500 uppercase p-2 text-center select-none">No Photo (Click to select/drag)</span>
                  )}
                </div>
              )}

              {side.textItems && side.textItems.map(item => {
                if (certificateData.showBottomPhoto && ['left_date_office', 'validity_label'].includes(item.id)) {
                  return null;
                }
